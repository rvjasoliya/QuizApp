...
class APIManager {
    ...
    func getRequestMethod(method: HTTPMethod = .get, url: String, parameters: Parameters = [:], success: @escaping APICompletionBlock) {
        showLoading()
        print(url)
        manager.request(url, method: method).responseJSON(completionHandler: { (response) in
            if response.response?.statusCode == 200 {
                let responseJson = JSON(response.value as Any)
                dissmissLoader()
                success(APIResponse.createSuccessAPIResponse(responseJson))
